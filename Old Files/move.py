...
SOURCE = Path(r"T:\Cards Done\Backed Up")
DEST = Path(r"G:\My Drive\Card Database")
DONE_CARDS = Path(r"T:\Card Database")
...
for game_folder in SOURCE.iterdir():
    ...
    for f in game_folder.iterdir():
        ...
        # Filename match found -> move source file into Card Database
        done_folder.mkdir(parents=True, exist_ok=True)
        try:
            shutil.move(str(f), str(done_folder / f.name))
            moved += 1
            print(f"  MOVED: {game} / {f.name} -> Card Database")
...
print(f"Moved to Card Database:     {moved}")
