...
def scrape_images(site_name, site_data, site_folder):
    ...
    driver = setup_driver()
    image_urls = []
    skipped_image_ids = read_skipped_csv(site_folder)
    ...
    image_urls = [url for url in image_urls if url not in skipped_image_ids]

    save_csv(image_urls, site_folder, "image_links.csv")
    print(f"[{site_name}] Scraping complete. {len(image_urls)} images saved.")

    # Check for duplicate image names
    image_names = [sanitize_filename(url.split("/")[-1]) + ".jpg" for url in image_urls]
